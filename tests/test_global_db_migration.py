...
def _make_parent(tmp_path: Path, data_path: Path) -> types.SimpleNamespace:
    ...
    (project / "jlcpcb").mkdir(parents=True)
...
def test_migrate_both_legacy_databases_into_global_db(
    ...
    """Copy corrections and preferences into global.db and remove both sources."""
...
def test_migrate_only_corrections_when_mappings_absent(
    ...
    data_path = tmp_path / "data"
    data_path.mkdir()
    _seed_corrections(data_path / "corrections.db", [("^C", 180, 0, 0)])
...
def test_migrate_only_mappings_when_corrections_absent(
    ...
    data_path = tmp_path / "data"
    data_path.mkdir()
    _seed_mappings(data_path / "mappings.db", [("R_0402", "1k", "C1")])
...
def test_interrupted_after_copy_before_delete_remigrates_and_removes(
    library_modules: Any, tmp_path: Path, caplog: pytest.LogCaptureFixture
) -> None:
    """Redo migration when global.db already has tables but the legacy file remains."""
    data_path = tmp_path / "data"
    data_path.mkdir()
    corrections = data_path / "corrections.db"
    global_db = data_path / "global.db"
    _seed_corrections(corrections, [("^legacy", 90, 1, 2)])
    # Simulate a prior copy that committed before the legacy file was removed.
    _seed_corrections(global_db, [("^stale", 0, 0, 0)])
    ...
    assert not corrections.exists()
    assert library.globalcorrectionsdb_file == str(global_db)
    assert _table_rows(global_db, "correction") == [("^legacy", 90, 1, 2)]
    assert "Replacing existing table 'correction'" in caplog.text
...
def test_mid_transaction_failure_leaves_legacy_and_rolls_back_destination(
    ...
    def fail_corrections(source: Path, destination: Path, log: logging.Logger) -> None:
        if source.name == "corrections.db":
            raise sqlite3.DatabaseError("simulated copy failure")
        original_copy(source, destination, log)
...
def test_empty_legacy_file_is_removed_and_paths_use_global_db(
    library_modules: Any, tmp_path: Path
) -> None:
...
def test_remove_sqlite_sidecars_with_legacy_file(
    library_modules: Any, tmp_path: Path
) -> None:
    """Delete WAL/SHM companions when removing a migrated legacy database."""
    global_db = library_modules.global_db

    data_path = tmp_path / "data"
    data_path.mkdir()
    corrections = data_path / "corrections.db"
    _seed_corrections(corrections, [("^W", 90, 0, 0)])
    wal = Path(f"{corrections}-wal")
    shm = Path(f"{corrections}-shm")
    ...
    assert result.uses_global("corrections.db")
    assert not corrections.exists()
