...
def _symbol(
    ...
    if version == 6:
        return f"""  (symbol (lib_id "Device:R") (at 0 0 0) (unit 1)
...
def _load_schematic(tmp_path, monkeypatch, version, paths, parts):
    """Load selected schematic paths using the requested KiCad version."""
    store = types.SimpleNamespace(read_all=lambda: parts)
    parent = types.SimpleNamespace(
        board_name="board.kicad_pcb", project_path=str(tmp_path), store=store
    )
...
def _run_export(
    ...
    if version == 6:
        (tmp_path / "board.kicad_sch").write_text(_v6_root(refs), encoding="utf-8")
...
def test_export_updates_only_the_base_bom_token(tmp_path, monkeypatch):
    """A later per-variant in_bom override remains untouched."""
    parts = [_part("RV2", "NEW", True), _part("RV99", "FOREIGN", False)]
    result = _run_export(tmp_path, monkeypatch, 8, "yes", ("RV2",), parts, variant=True)
...
def test_export_resolves_instances_in_empty_project(
    tmp_path, monkeypatch, secondary_excluded, expected_bom
):
...
def test_export_skips_unresolved_instances(
    tmp_path, monkeypatch, version, instance_text
):
